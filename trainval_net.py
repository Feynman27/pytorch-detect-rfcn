...
def parse_args():
  ...
  """
  parser = argparse.ArgumentParser(description='Train a rfcn siamese network')
...
if __name__ == '__main__':
  ...
  vid_train_size, det_train_size = len(roidb[0]), len(roidb[1])
  train_size = min(vid_train_size, det_train_size)

  #print('{:d} roidb frames'.format(len(roidb)))
  ...
  vid_dataset = roibatchLoader(roidb[0],ratio_list[0], ratio_index[0], 
          args.batch_size, imdb.num_classes, training=True)
  det_dataset = roibatchLoader(roidb[1],ratio_list[1], ratio_index[1], 
          args.batch_size, imdb.num_classes, training=True)
  # TODO Currently, some samples in the dataset with more roidb entries
  # will be discarded. Find a way to allow sampling of all roidb entries from 
  # both datasets. (e.g. N_A = 30k, N_B = 40k, 10k samples in B are discarded.)
  #dataloader = torch.utils.data.DataLoader(
  #        ConcatDataset(vid_dataset, det_dataset),
  ...
  if args.net == 'res101':
    RFCN = resnet(imdb.classes, 101, pretrained=True, class_agnostic=args.class_agnostic)
  elif args.net == 'res50':
    RFCN = resnet(imdb.classes, 50, pretrained=True, class_agnostic=args.class_agnostic)
  elif args.net == 'res152':
    RFCN = resnet(imdb.classes, 152, pretrained=True, class_agnostic=args.class_agnostic)
  else:
  ...
  for epoch in range(args.start_epoch, args.max_epochs):
    ...
    if args.mGPUs:
      save_name = os.path.join(output_dir, 'rfcn_detect_{}_{}_{}.pth'.format(args.session, epoch, step))
      save_checkpoint({
