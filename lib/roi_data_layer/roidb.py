...
def create_roi_pairs(roidb, training):
  '''
  ...
  '''
  roidb_frame_pairs = []
  for ientry in range(len(roidb)-1):
      ...
      if training:
        if (video_snippet1==video_snippet2) and (num_track_overlaps>0) and (flips_agree):
...
def combined_roidb(imdb_names, training=True):
  ...
  #  imdb = datasets.imdb.imdb(imdb_names, tmp.classes)
    ...
    #imdb = get_imdb(imdb_names)
      ...
      ratio_list=[]
      ratio_index = []
      for ir in range(len(roidbs)):
          if training:
            roidbs[ir] = filter_roidb(roidbs[ir])
            np.random.seed(123)
            np.random.shuffle(roidbs[ir])
          curr_ratio_list, curr_ratio_index = rank_roidb_ratio(roidbs[ir])
          ratio_list.append(curr_ratio_list)
          ratio_index.append(curr_ratio_index)
